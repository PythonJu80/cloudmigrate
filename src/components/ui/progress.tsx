...
>(({ className, value, showPercentage = true, ...props }, ref) => (
  <div className="flex items-center gap-3">
    ...
    >
      <ProgressPrimitive.Indicator
        className="h-full w-full flex-1 bg-terminal-green transition-all duration-300"
        style={{ transform: `translateX(-${100 - (value || 0)}%)` }}
      />
    </ProgressPrimitive.Root>
    {showPercentage && (
      <span className="text-xs font-mono text-terminal-green min-w-[3rem] text-right">
        {value || 0}%
      </span>
    )}
  </div>
