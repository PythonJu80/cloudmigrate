...
Uses local PostgreSQL instead of Supabase. No BYOK.
"""
import os
import sys
...
# Use local_db instead of shared_db
import local_db as db

# Get API key from environment (no BYOK)
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")
MODEL_CHOICE = os.getenv("MODEL_CHOICE", "gpt-4o-mini")


def get_openai_client() -> openai.OpenAI:
    """Get OpenAI client with environment API key."""
...
async def check_usage_limits_and_get_key(user_id: str) -> Dict[str, Any]:
    """Stub - always returns environment API key (no BYOK/limits)."""
    return {
