...
create table crawled_pages (
    ...
    -- Add a unique constraint to prevent duplicate chunks for the same URL per user
...
-- Create a policy that allows users to only see their own crawled_pages
...
-- Create a policy that allows users to only see their own sources
...
-- Create a policy that allows users to only see their own code_examples
create policy "Users can only access their own code_examples"
  on code_examples
  for all
  using (auth.uid() = user_id);